...
# Configuration
USE_VERTEX_AI = os.getenv('USE_VERTEX_AI', '').lower() in ('1', 'true', 'yes')
ANTHROPIC_API_KEY = os.getenv('ANTHROPIC_API_KEY')
VERTEX_PROJECT_ID = os.getenv('VERTEX_PROJECT_ID') or os.getenv('ANTHROPIC_VERTEX_PROJECT_ID')
...
if USE_VERTEX_AI:
    if not VERTEX_PROJECT_ID:
        raise ValueError("VERTEX_PROJECT_ID environment variable is required when USE_VERTEX_AI is enabled")
    proxy_logger.info("Running in Vertex AI mode")
    proxy_logger.info(f"Vertex AI Project ID: {VERTEX_PROJECT_ID}")
    proxy_logger.info(f"Vertex AI Region: {VERTEX_REGION}")
else:
    if not ANTHROPIC_API_KEY:
        raise ValueError("ANTHROPIC_API_KEY environment variable is required")
    proxy_logger.info("Running in Anthropic API mode")
...
# Different formats for Anthropic API vs Vertex AI
if USE_VERTEX_AI:
    # Vertex AI model format: claude-sonnet-4-5@20250929
    OLLAMA_TO_CLAUDE_MAPPING = {
        ...
        'claude-4-haiku:latest': 'claude-haiku-4-5@20251001'
    }
else:
    # Anthropic API model format: claude-4-sonnet-20250514
    OLLAMA_TO_CLAUDE_MAPPING = {
...
class ClaudeProvider:
    """Anthropic Claude API provider (supports both direct API and Vertex AI)"""

    def __init__(self, api_key: Optional[str] = None, use_vertex: bool = False,
                 vertex_project_id: Optional[str] = None, vertex_region: str = 'us-east5'):
        claude_logger.info("Initializing Claude provider")
        self.use_vertex = use_vertex

        if use_vertex:
            claude_logger.info(f"Using Vertex AI - Project: {vertex_project_id}, Region: {vertex_region}")
            self.client = AnthropicVertex(
                project_id=vertex_project_id,
                region=vertex_region
            )
            claude_logger.info("Vertex AI client initialized")
        else:
            self.api_key = api_key[:8] + "..." if api_key else None  # Log partial key for security
            claude_logger.info("Using Anthropic API")
            self.client = Anthropic(api_key=api_key)
            claude_logger.info(f"Anthropic client initialized - API key (partial): {self.api_key}")
    ...
    def chat(self, messages: List[Message], options: ChatOptions, model: str) -> ChatResponse:
        ...
        # Vertex AI doesn't allow both temperature and top_p, so only use temperature
        if self.use_vertex:
            if options.temperature != 0.7:  # If temperature is explicitly set
                request_params["temperature"] = options.temperature
            # Skip top_p for Vertex AI
        else:
            # For direct Anthropic API, both can be used
            request_params["temperature"] = options.temperature
        ...
        claude_logger.debug(f"[{request_id}] Request params keys: {list(request_params.keys())}")

        try:
            api_start_time = time.time()

            # Use Anthropic SDK (works for both direct API and Vertex AI)
            response = self.client.messages.create(**request_params)

            api_duration = time.time() - api_start_time

            claude_logger.info(
                f"[{request_id}] Claude API response received - Duration: {api_duration:.2f}s")
            ...
            claude_logger.error(f"[{request_id}] Error type: {type(e).__name__}")
            raise
